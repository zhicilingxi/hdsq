...
class CommonController extends Controller {
        ...
        public function _initialize() {
                ...
                //     exit();
                // }
                //var_dump($_SESSION[C('SESSION_USER_KEY')]);exit;


                if (empty($_SESSION[C('SESSION_USER_KEY')])) {
                        $url = C('scurl')['INDEX'].'/Admin/Public/login';
                        $str = "<script>var url = '{$url}';window.location.href = url;</script>";
                        exit($str);
                }

                if (isset($_SESSION[C('SESSION_USER_KEY')]['PromoterTel'])) {
                        $this->assign('Promoter', $_SESSION[C('SESSION_USER_KEY')]);
                }
        ...
        //采集列表
        public function caijiindex($source) {
                //列表过滤器，生成查询Map对象
                $map = $this->_search();
                if (empty($map)) {
                        $map['isupdate'] = 0;
                ...
                if ($source) {
                        $map['source'] = array('neq','0');
                }
                if (method_exists($this, '_cfilter')) {
                        $this->_cfilter($map);
                }
                //判断采用自定义的Model类
                if (!defined(CONTROLLER_NAME)) {
        ...
        protected function _bmlist($model, $map = array(), $sortBy = '', $asc = false) {
                ...
                $map['ID'] = array('not in','83491,83492,83493');
                //分页查询数据
                $list = $model->where($map)->order($order . ' ' . $sort)
                        ->limit($p->firstRow . ',' . $p->listRows)
                        ->select();
