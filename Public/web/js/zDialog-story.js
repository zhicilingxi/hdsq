...
Dialog.prototype.create = function () {
    ...
    var html = '\
  <table id="_DialogTable_' + this.ID + '" width="' + (this.Width + 26) + '" cellspacing="0" cellpadding="0" border="0" onselectstart="return false;" style="-moz-user-select: none; font-size:12px; line-height:1.4; background-color:#fff;  ">\
    <tr id="_Draghandle_' + this.ID + '" style="' + (this.Drag ? "cursor: move;" : "") + '">\
      <td width="20" ><div style="width: 20px;"></div></td>\
      <td  ><span style="font-size:16px;" id="_Title_' + this.ID + '">' + this.Title + '</span></div>\
        <div onclick="Dialog.getInstance(\'' + this.ID + '\').cancelButton.onclick.apply(Dialog.getInstance(\'' + this.ID + '\').cancelButton,[]);" onmouseout="this.style.backgroundImage=\'url(' + IMAGESPATH + 'closeBtn.png)\'" onmouseover="this.style.backgroundImage=\'url(' + IMAGESPATH + 'closeBtn.png)\'"  style="right:-8px; top:-12px; position:absolute; height: 30px; width: 30px;  cursor: pointer; background-image: url(' + IMAGESPATH + 'closeBtn.png);' + (this.ShowCloseButton ? "" : "display:none;") + '"></div></td>\
